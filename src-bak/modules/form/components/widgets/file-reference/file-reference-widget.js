...
export default class FileReferenceWidget extends BaseWidget {
    ...
    return (
      <div class='file-reference-wrapper'>
            ...
            h(EnList, {
              ...
              on: {
                definitionLoaded: (engineList) => {
                  ...
                  this.engineList.getContext().fileService.allowSelections(this.widgetSettings.allowed_types);
                }
              }
            })
          }
            ...
            <el-button type='primary' onClick={() => {
              ...
              if (selected.length === 0 || selected.length > 1) {
                EngineNotification.getInstance().showMessage({
                  type: 'error',
                  message: 'Please select one file, You have selected ' + selected.length + ' files'
                });
                return;
              }
        ...
        <div class='select-file'>
          <el-button type='primary' icon='el-file' class='file-reference-button' onClick={() => {
            this.showMediaPopup();
          }}>Select
          </el-button>
          <div class='el-upload__tip'>Select {this.widgetSettings.allowed_types.join(',')} </div>
        </div>
        <div class='preview'>
  ...
  renderPreview(h) {
    if (this.selectedFile) {
      ...
      if (type === 'image') {
        return <img class='img image-file el-avatar' style={{
          display: 'block',
          width: '50px',
          height: '50px'
  ...
  componentRender(component, h) {
    ...
    if (refModel) {
      ...
      if (!this.selectedFile) {
        this.selectedFile = Object.assign(new EngineFile(), { path: label, id: value, content_type: 'other' });
      }
    }
